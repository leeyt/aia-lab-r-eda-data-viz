...
# melt: 
# - 每一個測量變數獨占一個 row
# - 新增 key column 來表示對應的是哪個測量變數
# - 新增 value column 來表示測量值
long_DT <- melt(DT, id.vars = 5, measure.vars = 1:4, 
                variable.name = "key", value.name = "value")

# dcast: 
# - formula 左邊變量的 unique element 都會作為結果的一 row
...
long_DT <- melt(DT2, id.vars = 5:6, measure.vars = 1:4, 
                variable.name = "key", value.name = "value")
wide_DT <- dcast(long_DT, id + Species ~ key, 
                 value.var = "value", fun=NULL)
